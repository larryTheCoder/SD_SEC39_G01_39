                        ...
                        <nav className="bg-white border-gray-200 px-4 lg:px-6 py-2.5">
                            <div className="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
                                ...
                                <div className="flex items-center lg:order-2">
                                    <LoginButton/>
                                    <button data-collapse-toggle="mobile-menu-2" type="button" className="inline-flex items-center p-2 ml-1 text-sm text-gray-500 rounded-lg lg:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200"
                                ...
                                <div className="hidden justify-between items-center w-full lg:flex lg:w-auto lg:order-1" id="mobile-menu-2">
                                    <ul className="flex flex-col mt-4 font-medium lg:flex-row lg:space-x-8 lg:mt-0">
                                        <NavButtonStyle active={false} content="Home" location="/dashboard"/>
                                        <NavButtonStyle active={true} content="Edit Profile" location="/profile"/>
                                    </ul>
                                </div>
                            </div>
                        </nav>
...
function LoginButton() {
    ...
    return (
        <div className="ml-auto flex gap-2">
            {session?.user ? (
                <>
                    <img src="http://localhost:3000/api/profile-icon" className="mr-3 h-6 sm:h-9 rounded-full object-cover"/>
...
function NavButtonStyle({content, location, active}: {
    content: string
    location: string
    active: boolean
}) {
    return active ? (
        <a href={location} className="block py-2 pr-4 pl-3 text-white rounded bg-primary-700 lg:bg-transparent lg:text-primary-700 lg:p-0" aria-current="page">
            {content}
        </a>
    ) : (
        <a href={location} className="block py-2 pr-4 pl-3 text-gray-700 border-b border-gray-100 hover:bg-gray-50 lg:hover:bg-transparent lg:border-0 lg:hover:text-primary-700 lg:p-0">
            {content}
        </a>
    )
}
            ...
            <div className="w-full bg-white rounded-lg shadow md:mt-0 sm:max-w-md xl:p-0">
                <div className="p-6 space-y-4 md:space-y-4 sm:p-8">
                    ...
                    <PasswordInputBox onChange={(password) => {
                        ...
                        setShowNewPassword(password.length > 0)
