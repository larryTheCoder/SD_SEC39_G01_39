...
import {MAILGUN_API_KEY, MAILGUN_EMAIL_NAME, MAILGUN_PUBLIC_KEY} from "@/libs/config";

const mailgun = new Mailgun(formData);
const client = mailgun.client({username: 'api', key: MAILGUN_API_KEY, public_key: MAILGUN_PUBLIC_KEY});

export async function getEmailValidation(email: string) {
    return client.validate.get(email);
}

export async function sendPasswordResetMail(email: string, token: string) {
    const real_token = getDeploymentUrl() + "/reset/" + token;
    const result = await client.messages.create(MAILGUN_EMAIL_NAME, {
        from: `MudahMail <no-reply@${MAILGUN_EMAIL_NAME}>`,
        to: [email],
